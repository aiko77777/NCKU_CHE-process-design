...
## 📖 專案背景 (Background)
本專案為 **台灣化學工程學會 114年度 (2025-2026)** 化工程序設計競賽題目 。
目標是設計一個年產能 **10萬公噸 (100,000 MTA)** 的醋酸乙烯酯單體 (Vinyl Acetate Monomer, VAM) 製程 。

鑑於環保與能耗考量，本設計採用 **乙烯法 (Ethylene Process)**，其碳排 (2.5 t-CO2e/t VAM) 遠低於乙炔法 (6.0 t-CO2e/t VAM)，為目前主流生產技術 。

## ⚙️ 製程概述 (Process Description)
* **反應類型**：氣相氧化反應 (Gas-phase oxidation) 。
* **主要原料**：乙烯 (Ethylene)、醋酸 (Acetic Acid)、氧氣 (Oxygen) 。
* **催化劑系統**：金 (Au)、鈀 (Pd) 觸媒，並以醋酸鉀 (Potassium Acetate) 為活化促進劑。
* **反應方程式**：
    $$C_2H_4 + CH_3COOH + \frac{1}{2}O_2 \rightarrow CH_3COOCH=CH_2 + H_2O$$

## 🚀 設計要求與目標 (Design Requirements)
本專案報告涵蓋以下 12 項核心設計要求：

1.  **製程模擬**：包含反應、蒸餾、吸收、壓縮等單元，需提供詳細敘述 。
2.  **質能平衡**：計算 Mass/Energy Balance，並處理汽-液平衡 (VLE) 與不互溶液-液平衡 (LLE) 。
3.  **流程圖繪製**：PFD (Process Flow Diagram) 及控制流程圖，需標示物流編號及單位 (mass basis) 。
4.  **反應器安全設計**：反應器進出口氧氣濃度不得超過容許濃度的 **75% (安全餘裕)** 。
5.  **安全連鎖系統**：設計 Interlock System 以避免反應失控或燃燒爆炸 。
6.  **熱移除系統**：設計氧化反應熱之移除機制 。
7.  **CO2 移除**：設計系統移除因過度氧化產生的二氧化碳 。
8.  **原料回收循環**：
    * 移除不反應氣體 (Inert gas) 。
    * 未反應之乙烯與醋酸需經過處理後循環使用 。
9.  **副產物移除**：需從產品中移除醋酸甲酯、醋酸乙酯、乙醛及聚合物，以達產品純度 。
10. **聚合抑制**：設計添加適量抑制劑 (Inhibitor) 以防止 VAM 水解或聚合 。
11. **儲槽設計**：設計 VAM 產品儲槽，需考量避免自身聚合 。
12. **材質選用**：考量醋酸腐蝕性，選用適合的合金鋼材質。

## 📊 規格需求 (Specifications)

# 1. 產品規格 (VAM Product Spec) 
| 項目 | 規格 | 備註 |
| :--- | :--- | :--- |
| **VAM 純度** | **≥ 99.9 wt%** | |
| 水分 | ≤ 500 ppm | (0.05 wt%) |
